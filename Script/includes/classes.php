...
// Copyright (c) 2014 Pricop Alexandru - Mihai. All rights reserved.    \\
//----------------------------------------------------------------------\\
	...
	if($user !== false) {
		...
		*/
		$links = array(	array('profile&u='.$user['username'], realName($user['username'], $user['first_name'], $user['last_name']), 1, 0),
						...
						array('stream&logout=1', $LNG['admin_menu_logout'], 0, 0));
			...
			while(false !== ($theme = readdir($handle))) {
				...
				if($theme != '.' && $theme != '..' && file_exists('./'.$CONF['theme_path'].'/'.$theme.'/info.php')) {
					...
					if($CONF['theme_name'] == $theme) {
						...
						$state = '<div class="modal-btn list-button"><a href="'.$CONF['url'].'/index.php?a=admin&b=themes&theme='.$theme.'">'.$LNG['theme_activate'].'</a></div>';
					}
					...
					<div class="manage-users-container">
						...
						<div class="manage-users-content"><strong><a href="'.$url.'" target="_blank" title="'.$LNG['theme_author_homepage'].'">'.$name.'</a></strong> '.$version.'<br />'.$LNG['by'].': '.$author.'</div>
	...
	function getUsers($start) {
		...
		if($loadmore) {
			$output .= '<div class="admin-load-more"><div id="more_users">
					<div class="load_more"><a onclick="manage_the('.$last.', 0)">'.$LNG['load_more'].'</a></div>
				</div></div>';
		}
...
class managePayments {
	...
	function getPayments($start, $id = null) {
		...
		if($loadmore) {
			$output .= '<div class="admin-load-more"><div id="more_payments">
					<div class="load_more"><a onclick="manage_the('.$last.', 2)">'.$LNG['load_more'].'</a></div>
				</div></div>';
		}
	...
	function paymentButtons($payment) {
		...
		if($payment['status'] == 0) {
			$output .= '<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=payments&id='.$payment['id'].'&type=1">'.$LNG['enable'].'</a></div>';
		} else {
			$output .= '<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=payments&id='.$payment['id'].'&type=0">'.$LNG['suspend'].'</a></div>';
		}
...
class manageReports {
	...
	function getReports($start, $tracks) {
		...
		if($loadmore) {
			$output .= '<div class="admin-load-more"><div id="more_reports">
					<div class="load_more"><a onclick="manage_the('.$last.', 1)">'.$LNG['load_more'].'</a></div>
				</div></div>';
		}
	...
	function reportButtons($report) {
		...
		if($report['state'] == 0) {
			$output .= '<div class="page-inner">';
			if($report['type'] == 0) {
				$output .= '<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=reports&id='.$_GET['id'].'&type=2">'.$LNG['delete_comment'].'</a></div>
							<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=reports&id='.$_GET['id'].'&type=1">'.$LNG['delete_report'].'</a></div>';
			} else {
				$output .= '<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=reports&id='.$_GET['id'].'&type=3">'.$LNG['suspend_track'].'</a></div>
							<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=reports&id='.$_GET['id'].'&type=2">'.$LNG['delete_track'].'</a></div>
							<div class="modal-btn page-button"><a href="'.$this->url.'/index.php?a=admin&b=reports&id='.$_GET['id'].'&type=1">'.$LNG['delete_report'].'</a></div>';
			}
			$output .= '</div>';
		...
		// If the reported material has been deleted
			...
			$output = notificationBox('error', $LNG['suspended_track']);
		}
...
class feed {
	...
	function getTracks($query, $type, $typeVal) {
		...
		foreach($rows as $row) {
			...
			if($hide == 1 && !$this->is_admin) {
				...
				$track .= '<div id="track'.$row['id'].'" class="song-container'.(($type == 'trackPage') ? ' song-container-page' : '').'">
						<div class="song-art"><a href="'.$this->url.'/index.php?a=track&id='.$row['id'].'" rel="loadpage"><img src="'.$this->url.'/thumb.php?src='.$row['art'].'&t=m&w=112&h=112" id="song-art'.$row['id'].'" /></a></div>
						<div class="song-top">
							...
							<div class="song-titles">
								<div class="song-author"><a onmouseover="profileCard('.$row['idu'].', '.$row['id'].', 0, 0);" onmouseout="profileCard(0, 0, 0, 1);" onclick="profileCard(0, 0, 1, 1);" href="'.$this->url.'/index.php?a=profile&u='.$row['username'].'" rel="loadpage">'.realName($row['username'], $row['first_name'], $row['last_name']).'</a></div>
								<div class="song-tag">
									<a href="'.$this->url.'/index.php?a=explore&filter='.$tag.'" rel="loadpage">'.$tag.'</a>
								</div>
								<div class="song-title">
									<a href="'.$this->url.'/index.php?a=track&id='.$row['id'].'" id="song-url'.$row['id'].'" rel="loadpage"><div id="song-name'.$row['id'].'">'.$this->parseMessage($row['title']).'</div></a>
								</div>
							</div>
						</div>
				...
				$start = ($row['extra_id']) ? $row['extra_id'] : $row['id'];
			}
		}
		...
		if($loadmore) {
			$track .= '
						<div id="load-more">
							<div class="load_more"><a onclick="'.$type.'('.$start.', '.$typeVal.''.$profile.')">'.$LNG['load_more'].'</a></div>
						</div>';
		}
	...
	function explore($start, $value) {
		...
		if($value == 'popular music') {
			$query = sprintf("SELECT `views`.`track`, `tracks`.*, `users`.*, COUNT(`track`) as `count` FROM `views`,`tracks`,`users` WHERE `views`.`track` = `tracks`.`id` AND `tracks`.`uid` = `users`.`idu` %s AND DATE_SUB(CURDATE(),INTERVAL 7 DAY) <= date(`views`.`time`) GROUP BY `track` ORDER BY `count` DESC LIMIT %s", $start, ($this->per_page + 1));
			$value = '\''.$value.'\'';
		} elseif($value == 'liked music') {
			$query = sprintf("SELECT `likes`.`track`, `tracks`.*, `users`.*, COUNT(`track`) as `count` FROM `likes`,`tracks`,`users` WHERE `likes`.`track` = `tracks`.`id` AND `tracks`.`uid` = `users`.`idu` %s AND DATE_SUB(CURDATE(),INTERVAL 7 DAY) <= date(`likes`.`time`) GROUP BY `track` ORDER BY `count` DESC LIMIT %s", $start, ($this->per_page + 1));
			$value = '\''.$value.'\'';
		} elseif(!empty($value)) {
	...
	function stream($start, $value) {
		...
		if(in_array($value, $allowedDates)) {
			$query = sprintf("SELECT * FROM tracks, users WHERE tracks.uid = '%s' AND extract(YEAR_MONTH from `time`) = '%s' AND tracks.uid = users.idu %s ORDER BY tracks.id DESC LIMIT %s", $this->id, $this->db->real_escape_string($value), $start, ($this->per_page + 1));
			$value = '\''.$value.'\'';
		} else {
		...
		if($type) {
			...
			if($loadmore) {
				$output .= '<div id="load-more">
								<div class="load_more"><a onclick="loadPlaylists('.$value['id'].', 1, \''.$value['username'].'\')">'.$LNG['load_more'].'</a></div>
							</div>';
			}
	...
	function getPlaylistActions($id, $type = null) {
		...
		if($this->id == $get['by']) { // If it's current username is the same with the current author
			if($get['public'] == 1) {
				...
				$delete = '<div id="delete-button-'.$get['id'].'" class="delete-button" onclick="delete_modal('.$get['id'].', 3)" title="'.$LNG['delete_this_playlist'].'"></div>';
			} else {
		...
		$output = '<div class="playlist-actions-container">
						<div class="playlist-actions">
							<div class="playlist-actions-content"><div class="share-button" onclick="share('.$get['id'].', 2)"><span class="action-text">'.$LNG['share'].'</span></div>'.$delete.'<span id="privacy'.$get['id'].'">'.$privacy.'</span></div>
	...
	function fetchCategory($categories) {
		...
		if($_GET['a'] == 'explore' && !empty($_GET['filter']) && $_GET['filter'] !== 'popular music') {
	...
	function getTrack($id) {
		...
		$query = $this->db->query(sprintf("SELECT `idu`,`username`,`private`, `tracks`.`public` as `public` FROM `tracks`, `users` WHERE `tracks`.`id` = '%s' AND `tracks`.`uid` = `users`.`idu`", $this->db->real_escape_string($id)));
		$result = $query->fetch_assoc();
	...
	function getComments($id, $cid, $start, $type = null) {
		...
		if($result = $this->db->query($query)) {
			...
			if($loadmore) {
				$load = '<div class="load-more-comments" id="comments'.htmlentities($id, ENT_QUOTES).'"><div class="load_more"><a onclick="loadComments('.$message_id.', '.$load_id.', '.($start + $this->c_per_page).')">'.$LNG['load_more'].'</a></div></div>';
			}
	...
	function sidebarFilters($bold) {
		...
		foreach($row as $type) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'], ENT_QUOTES, 'UTF-8').'&filter='.$type.'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/filters/'.$type.'.png" />'.$LNG["sidebar_{$type}"].'</a></div>';
	...
	function sidebarNotifications($bold) {
		...
		$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/filters/all_notifications.png" />'.$LNG["all_notifications"].'</a></div>';
		if(empty($bold)) {
				$link .= '</strong>';
		}
		foreach($row as $type) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&filter='.$type.'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/filters/'.$type.'.png" />'.$LNG["sidebar_{$type}"].'</a></div>';
	...
	function sidebarCategories($bold) {
		...
		$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'" rel="loadpage">'.$LNG['latest_music'].'</a></div>';
		if(empty($bold)) {
				$link .= '</strong>';
		}
		...
		foreach(array('popular_music' => 'popular music', 'liked_music' => 'liked music') as $lang => $value) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&filter='.$value.'" rel="loadpage">'.$LNG[$lang].'</a></div>';
		...
		foreach($rows as $category) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&filter='.$category['name'].'" rel="loadpage">'.ucfirst($category['name']).'</a></div>';
	...
	function sidebarDates($bold, $values = null) {
		...
		if($row) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].$profile.'" rel="loadpage">'.$LNG["all_time"].'</a></div>';
			if(!$_GET['filter']) {
				$link .= '</strong>';
			}
			foreach($row as $date) {
				...
				// Output the links
				$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].$profile.'&filter='.$date.'" rel="loadpage">'.$LNG["month_{$month}"].' - '.$born[0].'</a></div>';
	...
	function sidebarReport($id) {
		global $LNG;
		return '<div class="sidebar-container sidebar-report"><div class="sidebar-description"><a '.(($this->id) ? 'href="'.$this->url.(($this->id) ? '/index.php?a=track&id='.$id.'&type=report' : '/index.php?a=welcome').'" rel="loadpage"' : 'href="#" onclick="connect_modal()"').'>'.$LNG['rci'].'</a></div></div>';
	}
	...
	function sidebarStatsFilters($bold) {
		...
		foreach($row as $type) {
			...
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].((isset($_GET['id'])) ? '&id='.$_GET['id'].'&type=stats' : '').'&filter='.$type.'" rel="loadpage">'.$LNG["stats_{$type}"].'</a></div>';
	...
	function getLikes($start = null, $type = null, $value = null) {
		...
		global $LNG, $CONF;
			if($type == 1) {
				...
				if($loadmore) {
					$output .= '<div id="load-more">
									<div class="load_more"><a onclick="loadLikes('.$last.', \''.$value.'\', \''.$type.'\')">'.$LNG['load_more'].'</a></div>
								</div>';
				}
	...
	function getSearch($start, $per_page, $value, $filter = null, $type = null) {
		...
		if($qt == 1) {
			...
			if($type) {
				...
				$music = $this->db->query(sprintf("SELECT `id`, `title` as `username`, `art` as `image`, 0 as `profile` FROM `tracks` WHERE `title` LIKE '%s' ORDER BY `id` DESC LIMIT %s, %s", '%'.$this->db->real_escape_string($value).'%', $this->db->real_escape_string($start), $per_page));
		...
		if($loadmore) {
				$output .= '<div id="load-more">
								<div class="load_more"><a onclick="loadPeople('.($start + $per_page).', \''.$value.'\', \''.$filter.'\')">'.$LNG['load_more'].'</a></div>
							</div>';
		}
	...
	function listSubs($type = null) {
		...
		if($loadmore) {
				$output .= '<div id="load-more">
								<div class="load_more"><a onclick="loadSubs('.$last.', '.$type.', '.$this->profile_data['idu'].')">'.$LNG['load_more'].'</a></div>
							</div>';
		}
		return $output;
	}
	...
	function getActions($id, $type = null) {
		...
		if($this->id == $get['uid']) { // If it's current username is the same with the current author
			if($get['public'] == 1) {
				...
				$delete = '<div id="delete-button-'.$get['id'].'" class="delete-button" onclick="delete_modal('.$get['id'].', 1)" title="'.$LNG['delete_this_track'].'"></div>';
			} else {
...
function statistics($db, $currency) {
	...
	(SELECT sum(`amount`) FROM `payments` WHERE DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= date(`time`) AND `status` = 1 AND `currency` = '%s') as earnings_last_30", $db->real_escape_string($currency), $db->real_escape_string($currency), $db->real_escape_string($currency), $db->real_escape_string($currency));
...
function welcomeTracks($rows, $url) {
	foreach($rows as $row) {
		$x .= '<div class="welcome-track"><a href="'.$url.'/index.php?a=track&id='.$row['track'].'" title="'.$row['title'].'" rel="loadpage"><img src="'.$url.'/thumb.php?src='.$row['art'].'&t=m&w=112&h=112"></a></div>';
	}
	return $x;
}
